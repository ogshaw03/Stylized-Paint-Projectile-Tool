...
def _resolve_latest_sha() -> str:
    """Ask the GitHub API for the current tip commit of the default
    branch. Falls back to the branch name if the API is unreachable."""
    import json
    import random
    import time
    ...
    api_url = f"{_GITHUB_API}/branches/{_GITHUB_BRANCH}?_={salt}"
    req = Request(api_url, headers={
        "Accept": "application/vnd.github+json",
        "Cache-Control": "no-cache",
        "User-Agent": f"paint_projectile-installer/{salt}",
    })
    try:
        with urlopen(req, timeout=30) as resp:
            payload = json.loads(resp.read().decode("utf-8"))
        sha = payload["commit"]["sha"]
        print(f"[paint_projectile] resolved {_GITHUB_BRANCH} -> {sha[:10]}")
        return sha
    except Exception as exc:
        print(f"[paint_projectile] SHA lookup failed ({exc}); "
              f"falling back to branch name")
...
def _download_from_github(dest_root: str) -> None:
    ...
    # a ?_=<salt> query string does NOT bust it. Fix: pin the download
    # to the latest commit SHA on main. SHA-scoped URLs are immutable,
    # so each new commit gives us a distinct URL that no cache can
    # confuse with a previous response.
    ...
    for rel_path in _REMOTE_FILES:
        ...
        try:
            req = Request(url, headers={
                "Cache-Control": "no-cache",
                "User-Agent": f"paint_projectile-installer/{sha[:10]}",
            })
...
#
# Uses the SHA-scoped raw URL rather than main/install.py because
# raw.githubusercontent.com's CDN caches by path only — a query-string
# cache-buster is a no-op there. The API lookup returns the tip SHA of
# main, then we hit the immutable /<SHA>/install.py URL.
_SHELF_UPDATE_CMD = (
    "# Auto-generated by paint_projectile install.py — do not edit.\n"
    "import json, urllib.request\n"
    "_api = 'https://api.github.com/repos/"
    "ogshaw03/Stylized-Paint-Projectile-Tool/branches/main'\n"
    "_sha = json.loads(urllib.request.urlopen(_api, timeout=30)"
    ".read())['commit']['sha']\n"
    "_u = 'https://raw.githubusercontent.com/"
    "ogshaw03/Stylized-Paint-Projectile-Tool/' + _sha + '/install.py'\n"
    "print('[paint_projectile] update via SHA', _sha[:10])\n"
